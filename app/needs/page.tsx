  ...
  const markPickedUp = async (id: number) => {
    ...
    try {
      const { error } = await supabase
        .from("community_needs")
        .update({ is_picked_up: true, status: "delivered" })
        .eq("id", id);
  ...
  if (!supabase) {
    return (
      <main className="min-h-screen bg-[#001E41] p-6 text-white font-sans">
        <Header title="Needs Command" subtitle="Mutual Aid Dashboard" isAdmin={true} />
            ...
            {needs.map((need) => (
              <tr key={need.id} className="border-b border-white/5 hover:bg-white/10 transition-colors">
                <td className="p-6">
                  ...
                  <div className="text-blue-400 font-black text-xs uppercase italic">
                    {need.location_area || "OKC Area"}
                  </div>
